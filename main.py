...
def main():
    ...
    ttk.Label(stego_frame, text="Input DOCX/DOCM/DOTM/DOTX:").grid(row=0, column=0, sticky="w")
    ttk.Entry(stego_frame, textvariable=file_in_stego, width=60).grid(row=0, column=1, padx=5)
    ttk.Button(stego_frame, text="Browse", 
               command=lambda: browse_file(file_in_stego, "Pilih DOCX/DOCM/DOTM/DOTX", WORD_TYPES)).grid(row=0, column=2)

    ttk.Label(stego_frame, text="Output (Embed):").grid(row=1, column=0, sticky="w", pady=5)
    ttk.Entry(stego_frame, textvariable=file_out_stego, width=60).grid(row=1, column=1, padx=5, pady=5)
    ttk.Button(stego_frame, text="Browse", 
               command=lambda: browse_save(file_out_stego, os.path.splitext(file_in_stego.get())[1] or ".docx")).grid(row=1, column=2)

    ttk.Label(stego_frame, text="Mode Payload:").grid(row=2, column=0, sticky="w", pady=5)
    ttk.Combobox(stego_frame, textvariable=mode_stego, values=["TEXT", "HASH", "JSON"], width=10).grid(row=2, column=1, sticky="w", pady=5)

    ttk.Label(stego_frame, text="Payload / Watermark:").grid(row=3, column=0, sticky="nw", pady=5)
    txt_payload = tk.Text(stego_frame, width=70, height=12)
    txt_payload.grid(row=3, column=1, columnspan=2, pady=5)

    def do_embed_stego():
        if not file_in_stego.get() or not file_out_stego.get():
            return messagebox.showwarning("Peringatan", "Pilih input dan output dokumen.")
        text_val = txt_payload.get("1.0", tk.END).strip()
        if not text_val:
            return messagebox.showwarning("Peringatan", "Isi payload dulu.")
        try:
            embed_stegano_office(file_in_stego.get(), file_out_stego.get(), text_val, mode_stego.get())
            messagebox.showinfo("Sukses", f"Payload berhasil di-embed ke:\n{file_out_stego.get()}")
        except Exception as e:
            messagebox.showerror("Error", str(e))

    def do_extract_stego():
        if not file_in_stego.get():
            return messagebox.showwarning("Peringatan", "Pilih dokumen dulu.")
        try:
            msg = extract_stegano_office(file_in_stego.get())
    ...
    btn_stego_frame.grid(row=4, column=1, columnspan=2, pady=10, sticky="e")
    ...
    ttk.Label(extractor_frame, text="Input DOCX/DOCM/DOTM/DOTX:").grid(row=0, column=0, sticky="w")
    ttk.Entry(extractor_frame, textvariable=file_in_extractor, width=50).grid(row=0, column=1, padx=5)
    ttk.Button(extractor_frame, text="Browse", 
               command=lambda: browse_file(file_in_extractor, "Pilih DOCX/DOCM/DOTM/DOTX", WORD_TYPES)).grid(row=0, column=2)

    def do_extract(func, title, not_found_msg):
        if not file_in_extractor.get():
            return messagebox.showwarning("Peringatan", "Pilih dokumen dulu.")
        out_dir = filedialog.askdirectory(title=f"Pilih folder untuk menyimpan {title}")
        if not out_dir:
            return
        try:
            outputs = func(file_in_extractor.get(), out_dir)
    ...
    ttk.Button(extractor_frame, text="Extract Embedded Objects", 
               command=lambda: do_extract(extract_embedded_objects, "embedded objects", "embedded objects")).grid(row=1, column=1, pady=10)
    ttk.Button(extractor_frame, text="Extract Hidden Payload After Footer", 
               command=lambda: do_extract(extract_hidden_after_footer, "hidden payload", "payload setelah footer ZIP")).grid(row=2, column=1, pady=10)
    ttk.Button(extractor_frame, text="Carve From Raw (Signatures)", 
               command=lambda: do_extract(carve_from_raw, "file carving", "signature yang cocok")).grid(row=3, column=1, pady=10)
